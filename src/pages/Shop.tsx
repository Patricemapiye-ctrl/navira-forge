...
const Shop = () => {
  ...
  const fetchProducts = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("inventory").select("id, item_name, item_code, description, category, quantity, unit_price, image_url").gt("quantity", 0).order("item_name");
    if (error) {
  ...
  const filterProducts = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(p => p.item_name.toLowerCase().includes(searchTerm.toLowerCase()) || p.item_code.toLowerCase().includes(searchTerm.toLowerCase()) || p.category.toLowerCase().includes(searchTerm.toLowerCase()));
    }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {user ? <>
                  ...
                  <ShoppingCart items={cart} onUpdateQuantity={updateCartQuantity} onRemoveItem={removeFromCart} onCheckout={handleCheckout} />
                </> : <>
                  <Button onClick={() => {
                setAuthMode("login");
                setShowAuthDialog(true);
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search products..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-hardware-steel/20 border-primary/20 text-white" />
              </div>
              <div className="w-full md:w-64">
                <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                  ...
                  <SelectContent className="bg-hardware-dark border-primary/20">
                    {categories.map(category => <SelectItem key={category} value={category} className="text-white hover:bg-hardware-steel/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map(product => <Card key={product.id} className="bg-hardware-dark/50 border-primary/20 hover:border-primary/50 transition-all group">
              ...
              <CardContent>
                ...
                <Button onClick={() => addToCart(product)} className="w-full bg-primary hover:bg-primary/90" disabled={product.quantity === 0}>
        ...
        <DialogContent className="bg-hardware-dark border-primary/20">
          <DialogHeader>
            ...
            <DialogDescription className="text-hardware-light">
              {authMode === "signup" ? "Create an account to complete your purchase" : "Login to continue shopping"}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            {authMode === "signup" && <div>
                <label className="text-sm text-hardware-light mb-1 block">Name</label>
                <Input type="text" placeholder="Your name" value={name} onChange={e => setName(e.target.value)} className="bg-hardware-steel/20 border-primary/20 text-white" />
              </div>}
            <div>
              <label className="text-sm text-hardware-light mb-1 block">Email</label>
              <Input type="email" placeholder="your@email.com" value={email} onChange={e => setEmail(e.target.value)} className="bg-hardware-steel/20 border-primary/20 text-white" />
            </div>
            <div>
              <label className="text-sm text-hardware-light mb-1 block">Password</label>
              <Input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="bg-hardware-steel/20 border-primary/20 text-white" />
            </div>
            <Button onClick={handleAuth} className="w-full bg-primary hover:bg-primary/90">
              {authMode === "signup" ? "Create Account" : "Login"}
            </Button>
            <div className="text-center">
              <button onClick={() => setAuthMode(authMode === "signup" ? "login" : "signup")} className="text-sm text-primary hover:underline">
