...
const SalesPoint = () => {
  ...
  const fetchItems = async () => {
    const {
      data
    } = await supabase.from("inventory").select("*").gt("quantity", 0).order("item_name");
    setItems(data || []);
  };
  const addToCart = () => {
    ...
    if (existingItem) {
      setCart(cart.map(i => i.inventory_id === item.id ? {
        ...i,
  ...
  return <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={selectedItem} onValueChange={setSelectedItem}>
                ...
                <SelectContent>
                  {items.map(item => <SelectItem key={item.id} value={item.id}>
        ...
        <CardContent>
          {cart.length === 0 ? <p className="text-center text-muted-foreground py-8">Cart is empty</p> : <>
              ...
              <div className="mt-4 space-y-2">
                ...
                <Button onClick={handleCompleteSale} className="w-full bg-primary hover:bg-primary/90" size="lg">
                  Complete Sale
                </Button>
              </div>
            </>}
      ...
      {lastSale && <div className="lg:col-span-2">
